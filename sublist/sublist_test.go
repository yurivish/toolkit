...
func newSub(subject string) *Subscription {
	return &Subscription{Subject: []byte(subject), Value: subject}
}

func newQSub(subject, queue string) *Subscription {
	if queue != "" {
		return &Subscription{Subject: []byte(subject), Queue: []byte(queue), Value: subject + ":" + queue}
	}
	return newSub(subject)
}

func newRemoteQSub(subject, queue string, num int32) *Subscription {
	// For our simplified version, remote queue subs are just regular queue subs
	// The original NATS implementation used these for cluster routing
	return newQSub(subject, queue)
}
...
// Commented out: ValidateMapping is part of NATS subject transformation features
...
// We do not use remote subscriptions, so we omit their tests.
...
func TestSublistAll(t *testing.T) {
	...
	// Commented out: our simplified Subscription doesn't have client field
...
// checkFor is a test helper that polls a condition function
func checkFor(t testing.TB, dur, interval time.Duration, fn func() error) {
	t.Helper()
